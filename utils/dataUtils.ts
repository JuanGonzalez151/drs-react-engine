...
import { ColumnProfile, ColumnType, DataRow, DatasetStats, SemanticType, ChartConfig } from '../types';
...
export const prepareVisualData = (data: DataRow[], config: ChartConfig): any[] => {
  ...
  const distinctX = new Set(data.map(r => r[xKey])).size;

  // A) Binning logic for Numeric X with high cardinality (e.g. Age)
  if (isXNumeric && distinctX > 20) {
    const values = data.map(r => r[xKey] as number).filter(n => !isNaN(n));
    const min = Math.min(...values);
    const max = Math.max(...values);
    const binCount = 10;
    ...
    workingData = data.map(row => {
       ...
       const binStart = Math.floor(min + (binIndex * binSize));
       const binEnd = Math.floor(min + ((binIndex + 1) * binSize));
       // Store new binned key
       return { ...row, [`${xKey}_binned`]: `${binStart}-${binEnd}` };
    });
    ...
    if (yKeys.length === 0) {
        ...
        yKeys.forEach(yKey => {
            if (isNumeric(yKey)) {
                ...
                result[yKey] = parseFloat((sum / rows.length).toFixed(2));
            } else {
                // Categorical Target: COUNT (Frequency)
                // e.g. Engagement Level. We can't average "High". We count rows.
                // This effectively makes the Bar height = Count.
